...
function checkForIncludes(str) {
	...
	// check for includes
	let match = str.match(/({{){1,}[^a-zA-Z0-9]*include ([a-zA-Z0-9_.-]+.html){1}[^a-zA-Z0-9]*(}}){1,}/gi);
	...
	match.forEach(function(element) {
		...
		// get file depending on the length of array after split
		if(elArray.length === 3 || elArray.length === 4) file = elArray[2].replace(/}/g, '');
		else file = elArray[1].replace(/}/g, '');

		// read file
		let newFile = fs.readFileSync(`${viewFolder}/${file}`, 'utf8');

		// replace regular expression
		let replaceStr = new RegExp(`({{){1,}[^a-zA-Z0-9]*include (${file}){1}[^a-zA-Z0-9]*(}}){1,}`, 'g');
